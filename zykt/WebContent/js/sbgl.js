...
$(document).ready(function() {
		...
		function jsArrChange(json){
			for (var i = 0 ; i < json.length ; i ++) {
				...
				arr1[15] = json[i].time;
				arr1[16] = json[i].gdtime;
				arr1[17] = json[i].zdtime;
				arr1[18] = json[i].ddtime;
				arr1[19] = json[i].dgdtime;
				arr1[20] = json[i].dzdtime;
				arr1[21] = json[i].dddtime;
...
//		某户所以风盘操作
	/*	$("#operation_all_btn").click(function(){
	    ...
	    function tableToExcel(){
	        ...
	        '<td>楼栋</td><td>单元</td><td>户号</td><td>缴费状态</td>'+
	        '<td>设定温度</td><td>出风温度</td>'+
	        '<td>已用当量</td><td>总金额</td><td>已用金额</td><td>剩余金额</td><td>档位</td><td>采集时间</td>'+
	        '<td>制冷高档</td><td>制冷中档</td><td>制冷抵挡</td>'+
	        '<td>制热高档</td><td>制热中档</td><td>制热抵挡</td>'+
	        '<td>用户面积</td><td>用户电话</td><td>计费状态</td><td>运行状态</td>'+
		...
		function search(page){
			...
			tbodydis(xinwordList,compareWordList,page);
		}	
});
